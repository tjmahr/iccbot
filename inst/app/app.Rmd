...
##### **Repeated raters (two-way) vs. unique raters (one-way)**
  ...
  - If raters only ever evaluate one participant each, use `no (one-way
    model)`.
...
##### **Agreement vs. Consistency**
  ...
  - If the instrument is a standardized or diagnostic test, and scores
    are interpreted with respect to age or sex norms, use `absolute
    agreement`.
  - If (just) the rankings of participants should be similar between
    raters, use `consistency`.
  - If the raters are unique, as in `no (one-way model)`, only
...
there is no way to assess whether one rater is a harder scorer on
average compared to another rater. Therefore, only consistency is
...
a single score. This situation requires a `one-way, consistency-based,
average rating` ICC.
...
per child. This situations requires a `two-way, agreement-based, single
rating` ICC.
...
<div id="ref-kreft1998introducing">
